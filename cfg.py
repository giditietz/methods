...
def generate_random_query_test(max_hight=2):
    """
...
def main():
    ...
    variables = [year, movie_length, rating, genre,
                 ...
                 contains_title_syllable, starts_actor_syllable, starts_country_syllable,starts_director_syllable,
    ...
    while running:
        ...
        if user == 1 or user == 2:
            while True:
                ...
                if user == 2:
                    comp = unsat
                z3_query, real_query = generate_random_query_test(3)
                s = Solver()
            ...
            index = 0
            use_file = False
            runs = input("Please choose the size of the dataset:\n")
            if runs > 50:
                ask_for_file = input("The chosen size is quite big - would you \n"
            ...
            if use_file:
                file.write("{0}\t{1}\t{2}\t{3}\n".format("index", "result", "length", "bucket"))
            else:
                print("{0}\t{1}\t{2}\t{3}".format("index", "result", "length", "bucket"))
            while (index < runs):
                query, _ = generate_random_query_test(max_hight)
                if (len(query) < 2000):
                ...
                if (res == unsat):
                    ...
                    sats += 1
                    res = 0
                bucket = (len(query) / 100) * 100
                if use_file:
                    file.write("{0}\t\t{1}\t\t{2}\t{3}\n".format(index, res, len(query),bucket))
                else:
                    print("{0}\t\t{1}\t\t{2}\t{3}".format(index, res, len(query), bucket))
                index += 1
